...
const SafeConverter = () => {
    ...
    const calculateValuationCapShares = (safeInput: SafeInput): number => {
  ...
  const calculateDiscountShares = (safeInput: SafeInput): number => {
      if (safeInput.type === 'Discount SAFE' && safeInput.discountRate !== undefined) {
          const discountFactor = (100 - safeInput.discountRate) / 100;
          return safeInput.investmentAmount / (equityFinancingValuation * discountFactor) * totalShares;
      }
      return 0;
  };
  ...
  const calculateEquityShares = (safeInput: SafeInput): number => {
      if (safeInput.type === 'Valuation Cap SAFE') {
          return calculateValuationCapShares(safeInput);
      } else if (safeInput.type === 'Discount SAFE') {
          return calculateDiscountShares(safeInput);
      }
      return 0;
  };
